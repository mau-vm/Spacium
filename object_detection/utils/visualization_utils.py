...
def visualize_boxes_and_labels_on_image_array(
  ...
  for box, color in box_to_color_map.items():
    ...
    cntpersons += 1  #<-----
    #if (ymax-ymin) <= 0.48 and (ymax-ymin) > 0.1: #<-----
    #    person += 1 #<-----

    #print(abs(slope))
    if abs(slope)  >= threshsit: #<----
      person += 1#<---
      slope = 0 #<----
      evaluacion.append(person)
  ...
  if len(evaluacion) < numEvaluaciones:
    person = auxpersonas
  else:
    contadorVal = 0

    for i in range(len(evaluacion)):
      contadorVal = evaluacion.count(evaluacion[i])
      if contadorVal >= minValores:
        valores.append(evaluacion[i])
    person = max(valores)
    auxpersonas = max(valores)
    evaluacion = []
    valores = []

  disponibles = cntAsientos - person
  if disponibles < 0:
    disponibles = 0

  cv2.rectangle(image, (10,5), (210, 60), (0, 186, 251), -1, cv2.LINE_AA) #<-----
  font = cv2.FONT_HERSHEY_DUPLEX #<-----
  #cv2.putText(image,'>',(545,22), font, 0.7,(163,73,164),1,cv2.LINE_AA) #<-----
  #cv2.putText(image,'accenture',(485,40), font, 0.7,(0,0,0),1,cv2.LINE_AA) #<-----
  cv2.putText(image,'Semana i Challenge',(475,53), font, 0.4,(255,255,255),1,cv2.LINE_AA) #<-----
  cv2.putText(image,'Personas: '+str(cntpersons),(15,20), font, 0.5,(0,0,0),1,cv2.LINE_AA) #<-----
  cv2.putText(image,'Lugares Ocupados: '+str(person),(15,35), font, 0.5,(0,0,0),1,cv2.LINE_AA) #<-----
  cv2.putText(image,'Lugares Disponibles: '+str(disponibles),(15,50), font, 0.5,(0,0,0),1,cv2.LINE_AA) #<-----
  return image
